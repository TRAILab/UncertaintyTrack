...
def acc_single_video(results,
    ...
    frame_ids = [np.full(len(results), -1) for i in range(num_classes)]
    
    #! first frame a class appears is noted as frame 0; not actually frame 0
    for j, (result, gt) in enumerate(zip(results, gts)):
        ...
        for i in range(num_classes):
            ...
            if dist.shape != (0, 0):
                ...
                frame_ids[i][j] = accumulators[i].mot_events.index[-1][0]
    return accumulators, frame_ids
...
def get_mot_acc(results,
                ...
                ignore_by_classes=False,
                nproc=4):
    """Get MOT accumulators from MOT evaluator to distinguish FP/FN/IDSW.
    ...
    Returns:
        accs (motmetrics.mot.MOTAccumulator): MOT events.
    """
